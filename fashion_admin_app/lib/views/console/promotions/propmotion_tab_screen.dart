...
class PropmotionTabScreen extends StatelessWidget {
  const PropmotionTabScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(body:Consumer<AdminProviders>(builder: (context,value,child){
      if (value.isLoading) {
            return const Center(
           ...
           List<PromoModel> promos =
              PromoModel.fromJsonList(value.promos);
               if (promos.isEmpty) {
            return const Center(
        ...
        onPressed: () {
          showDialog(
            context: context,
            builder: (context) => AddModifyPromos(isUpdating: false, promoId: "")
          );
        },
      ),);
  }
